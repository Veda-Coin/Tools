...
if [ -n "$(pidof $VEDA_DAEMON)" ] || [ -e "$VEDA_DAEMOM" ] ; then
  echo -e "${GREEN}\c"
  read -e -p "Veda is already installed. Do you want to add another MN? [Y/N]" NEW_VEDA
  echo -e "{NC}"
  clear
else
  NEW_VEDA="new"
...
function create_config() {
  ...
  RPCPASSWORD=$(pwgen -s 15 1)
    ...
    PRIV_KEY=$(veda-cli masternode genkey)
...
EOF
  ...
  outputs=$(veda-cli masternode outputs)
  a=${outputs#*\"}
  tr_hash=${a%%\"*}
...
sleep 5
MNSYNCSTAT=1
 echo -e "${GREEN}Please wait until mansernode syncronization!${NC}"
while [ $MNSYNCSTAT -lt 999 ]; do
    sleep 1
    output=$(veda-cli mnsync status)
    a=${output#*\"}
    a=${a#*\"}
    a=${a%%\,*}
    MNSYNCSTAT=${a#*\ }
...
function create_masternode_config() {
  ...
  IP_ADDRESS=$($IP_COMMAND)
    echo $IP_ADDRESS
  cat << EOF > $DEFAULTVEDAFOLDER/$MASTERNODE_CONFIG_FILE
$MN_NAME $IP_ADDRESS:$DEFAULTVEDAPORT $PRIV_KEY $tr_hash 0
